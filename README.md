# ✨ ZU X ORI Wallpapers Vault

This repository will house a growing collection of **ZU X ORI** wallpapers — designed for desktop and mobile — featuring the characters, symbols, and storyworld of the reincarnation epic.

---

## 🖼️ What's Coming

- High-resolution desktop wallpapers  
- Mobile-optimized vertical posters  
- Character close-ups  
- Symbolic + thematic art  
- Seasonal and scene-based designs  

All visuals will reflect the mood, mystery, and memory-driven themes of ZU X ORI.

---

## 🔓 Usage & Attribution

All wallpapers in this repo are released into the **public domain (CC0)** — feel free to download, remix, share, or use them however you like.

> ✨ Attribution is not required, but always welcome:
>  
> `"ZU X ORI" (zuxori.com) — from the official wallpaper archive.`

---

## 🔗 Explore More

- 📖 [Story Repository](https://github.com/zuxori/story)  
- 🌌 [Characters](https://github.com/zuxori/characters)  
- 🖼 [Images & Logos](https://github.com/zuxori/images)  
- 📜 [Research Papers](https://github.com/zuxori/research)  
- 🌐 [Website](https://zuxori.com)

---

> “not sure why i saved it. but i did.”
